...
class _PregnancyProgressState extends State<PregnancyProgress> {
  ...
  TableCalendar createTableCalendar() {
    return TableCalendar(
                ...
                calendarBuilders: CalendarBuilders( // Build the calendar
                  headerTitleBuilder: (context, day) { // Build the header of the calendar
                    return Row( 
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text( // Display current month and year
                          '${getMonth(day.month)} ${day.year}', 
                          style: TextStyle(color: Colors.purple[300], fontSize: 20, fontWeight: FontWeight.bold),
                        ),
                        IconButton( // Icon for adding an event 
                          icon: Icon(Icons.add, color: Colors.purple[200],), 
                          onPressed: () => showAddEventDialog('Add New Event', addEvent),)
                      ]
                    );
                  },
                ),
  ...
  Row createToDoAdder() {
    return Row(
                  ...
                  children: [
                    const Checkbox( // A checkbox that does nothing
                      value: false,
                      onChanged: null,
                    ),
                    SizedBox( // Text field for user to type in a to do
                        height: 45,
                    ...
                    SizedBox( // Icon to add the to do item to the list
                        width: 50,
                        child: IconButton(
  ...
  Expanded showToDoItems() {
    return Expanded(
                child: ListView.builder( // length of the to do list
                    ...
                    itemBuilder: (context, index) {
                      return ListTile(
                          leading: Checkbox( 
                              ...
                              onChanged: (value) => // If the checkbox is checked update it to true in db
                          ...
                          trailing: IconButton(
                            icon: const Icon(Icons.delete), // Icon to delete the to do item if pressed
  ...
  Future<void> _onDaySelected(DateTime day, DateTime focusedDay) async {
    await Provider.of< PregnancyModel>(context, listen: false).selectEvents(day); // set the all the events from that day in the model
    setState(() {
      today = day; // Just so that the day you select is visibly shown in calendar
      showDialog( // Show the popup
        context: context,
  ...
  void showAddEventDialog(String dialogTitle, Function addOrUpdate, {String event = 'Title', String? location = 'Location', String date = 'Date', String time = 'Time'})  {
    if(dialogTitle.contains('Update')) { // If the dialog is supposed to be an update
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              TextFormField( // For the title of the event
              ...
              TextFormField( // For the location of the event
              ...
              const SizedBox(height: 20),
              TextFormField( // For the date of the event
              ...
              const SizedBox(height: 20),
              TextFormField( // For the time of the event
  ...
  Future<void> popUpDatePicker() async {
    final DateTime? datePicked = await showDatePicker( // Show the date picker
        context: context, 
        firstDate: DateTime.now(), // Default for the first day
        lastDate: DateTime(today.year + 1, DateTime.december, 0)); // last day is december of next year

    setState(() {
        selectedEventDateTime = datePicked; // Set the date that is selected and show it in the textfield
  ...
  void addEvent() { 
    Provider.of<PregnancyModel>(context, listen: false).addEvent( // Add event to the database
      eventTitleTextFieldController.text, 
      eventLocationTextFieldController.text,
      selectedEventDateTime);
    
    setState(() { // Clear the text fields and pop off
  ...
  Future<void> updateEvent(int eventID) async { 
    ...
    setState(() { // Clear the text fields and pop off
      ...
      //TODO: make sure when you pop off, the event is actually updated..when popping off, it doesnt rebuild?
      Navigator.of(context).pop();
    });
  }
        ...
        itemBuilder: (context, index) {
          return ListTile(
            leading: IconButton(
              ...
              onPressed: () => showAddEventDialog('Update Event', 
              () async => await updateEvent(
  ...
  String formatStringTime(String time) {
    ...
    String aMpM = (int.parse(timeSplit[0]) > 12) ? "PM" : "AM"; // If the hour is more than 12 its PM
  ...
  String convertToMilitaryTime(String time) {
    List timeSplit = time.split(' '); // Split by space to separate the time from AM or PM
    List hourMin = timeSplit[0].split(':'); // Split to separate hour and min
    String hour = hourMin[0]; 
    String min = hourMin[1];

    if(timeSplit[1] == 'PM') { // If its pm, add 12 to the hour
      hour = '${int.parse(hour[0]) + 12}';
    }

    if (hour.length < 2) { // if the hour is less than 2 digits, pad it with a 0
      hour = '0$hour';
    }
