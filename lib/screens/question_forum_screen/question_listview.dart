...
class _QuestionListViewState extends State<QuestionListView> {
    ...
    if (initialized) {
      switch (choice) {
        case 'Newest':
          {
            setState(() {
              widget.questionForumModel.sortQuestionByNewest();
            });
          }
          break;
        case 'Oldest':
          {
            setState(() {
              widget.questionForumModel.sortQuestionsByOldest();
            });
          }
          break;
        case 'Most Popular':
          {
            setState(() {
              widget.questionForumModel.sortQuestionsByMostPopular();
            });
          }
          break;
        case 'Least Popular':
          {
            setState(() {
              widget.questionForumModel.sortQuestionsByLeastPopular();
            });
          }
          break;
      }
  ...
  void seeMyQuestions() {
    // go to the archive and view your questions
    if (supabaseModel.supabase!.auth.currentUser?.id == null) {
      ...
      Navigator.of(context)
          .push(MaterialPageRoute(
              builder: (context) => Consumer<QuestionForumModel>(
                  builder: (context, questionsChangeNotifier, child) =>
                      MyQuestions(
        ...
        children: [
          Expanded(
              child: ListView.separated(
                  // builder shows all the questions that have been asked
                  itemCount: widget.questionForumModel.questionsList.length,
                  itemBuilder: (context, index) {
                    return (supabaseModel.supabase!.auth.currentUser?.id ==
                            widget.questionForumModel.questionsList[index]
                                .getUserWhoPosted())
                        ? currentUserQuestion(index)
                        : notCurrUserQuestion(index);
                  },
  ...
  ListTile currentUserQuestion(int questionIndex) {
    isLiked = widget.questionForumModel.likedQuestionsList.contains(
        widget.questionForumModel.questionsList[questionIndex].questionID);
    var futureAuthor =
        widget.questionForumModel.questionsList[questionIndex].getAuthor();
    ...
    return ListTile(
      title: Text(
          widget.questionForumModel.questionsList[questionIndex].getQuestion(),
          style:
              const TextStyle(fontWeight: FontWeight.bold)), // prints question
      subtitle: (author ==
              null)
          ? Text(
              "Anonymous ● ${widget.questionForumModel.questionsList[questionIndex].date.toString().substring(0, 10)}")
          : Text(
              "${author} ● ${widget.questionForumModel.questionsList[questionIndex].date.toString().substring(0, 10)}"),
      trailing: Row(mainAxisSize: MainAxisSize.min, children: [
        likeButton(questionIndex),
        IconButton(
            onPressed: () {
              _deleteQuestionVerification(questionIndex);
            },
  ...
  ListTile notCurrUserQuestion(int questionIndex) {
    ...
    return ListTile(
        title: Text(
            widget.questionForumModel.questionsList[questionIndex]
                .getQuestion(),
            style: const TextStyle(
                fontWeight: FontWeight.bold)), // prints question
        subtitle: (author ==
                null)
            ? Text(
                "Anonymous ● ${widget.questionForumModel.questionsList[questionIndex].date.toString().substring(0, 10)}")
  ...
  SizedBox likeButton(int questionIndex) {
    return SizedBox(
      ...
      child: LikeButton(
          ...
          bubblesSize: 0,
          likeCount:
              widget.questionForumModel.questionsList[questionIndex].numLikes,
          isLiked: isLiked,
          onTap: (bool isLiked) {
            if (supabaseModel.supabase!.auth.currentUser?.id == null) {
              _showNoAccountWarning();
  ...
  void _addQuestion() {
    Navigator.of(context)
        .push(MaterialPageRoute(
            builder: (context) =>
                AddQuestion(questionForumModel: widget.questionForumModel)))
        .then((_) => setState(() {
              widget.questionForumModel.loadQuestionList();
            }));
  }
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content:
              const Text("You must have an account to perform this action."),
