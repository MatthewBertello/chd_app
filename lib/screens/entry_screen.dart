...
class EntryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List entries = Provider.of<VariableEntriesModel>(context).getVariableEntriesFromDate(date: startDate, entries: Provider.of<VariableEntriesModel>(context).variableEntries);
    entries.sort((entry1, entry2) => entry1['date'].compareTo(entry2['date']));
    return Scaffold(
      appBar: DefaultAppBar(context: context, title: Text(DateFormat.yMd().format(startDate))),
